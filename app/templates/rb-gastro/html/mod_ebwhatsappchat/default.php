...
<style type="text/css">
	...
	.whatsappchat_<?php echo $module_id; ?> .popup .myPopup_<?php echo $module_id; ?>::after { border-color: transparent transparent <?php echo $wsac_bgcolor; ?> transparent; }
	<?php } else if ($wsac_icnpst == 'top_right') {?>
	.whatsappchat_<?php echo $module_id; ?> .popup .myPopup_<?php echo $module_id; ?>::after { border-color: transparent transparent <?php echo $wsac_bgcolor; ?> transparent; }
	<?php } else if ($wsac_icnpst == 'bottom_left') {?>
		.whatsappchat_<?php echo $module_id; ?>.is-bottom_left .popup .myPopup_<?php echo $module_id; ?>::after { border-color: <?php echo $wsac_bgcolor; ?> transparent transparent transparent; }
	<?php } else if ($wsac_icnpst == 'bottom_right') {?>
		.whatsappchat_<?php echo $module_id; ?>.is-bottom_right .popup .myPopup_<?php echo $module_id; ?>::after { border-color: <?php echo $wsac_bgcolor; ?> transparent transparent transparent; }
	<?php }?>
	.whatsappchat h3 { color: <?php echo $wsac_txtcolor ?> }
</style>
		<div class="is_<?php echo $wsac_icnpst; ?> whatsappchat_<?php echo $module_id; ?> whatsappchat is-<?php echo $wsac_icnpst; ?>">
			<div class="mod-whatsapp">
				<?php if ($wsac_icnimg == "upload_icon") {?>
					<span class="btn-whatsapp help_btn_<?php echo $module_id; ?> help-you-btn">
					...
					<?php } else {?>
						<span class="btn-whatsapp help_btn_<?php echo $module_id; ?> help-you-btn" style="background-color: <?php echo $wsac_bgcolor ?>; color: <?php echo $wsac_txtcolor ?>">
						<?php if ($icon_img != '') {?><img src="<?php echo $icon_img; ?>" class="is-analytics" id="text_iconimg" alt="WhatsApp" /><?php }?>
						<?php echo $wsac_dsplytxt ?>
					</span>
					<?php }?>
				<?php }?>
				<span class="myPopup_<?php echo $module_id; ?> popup" id="myPopup">
					...
					<div class="response">
						...
						<a href="javascript:void(0);" class="submit_btn_<?php echo $module_id; ?> send_btn">
							<img src="<?php echo $mdl_path . '/assets/images/send-img.png'; ?>">
						</a>
					</div>
				</span>
			</div>
	 	...
	 	jQuery(function(){
			 jQuery(".is_<?php echo $wsac_icnpst; ?>").each(function(i){
			 	...
			 	if(icon_position == "bottom_left" || icon_position == "bottom_right"){
			 		jQuery(this).css('bottom', sum+'rem');
			 	}
			 	if(icon_position == "top_left" || icon_position == "top_right"){
			 		jQuery(this).css('top', sum+'rem');
			 	}
