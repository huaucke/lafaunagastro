...
<form id="finder-search" action="<?php echo JRoute::_($this->query->toUri()); ?>" method="get" class="form-inline">
	...
	<fieldset class="mod-finder">
		...
		<?php if ($this->escape($this->query->input) != '' || $this->params->get('allow_empty_search')):?>
			<button name="Search" type="submit" class="btn-search"><i class="icon-search"></i></button>
		<?php else: ?>
			<button name="Search" type="submit" class="btn-search disabled"><i class="icon-search"></i></button>
		<?php endif; ?>
<!--		<?php if ($this->params->get('show_advanced', 1)) : ?>
			<a href="#advancedSearch" data-toggle="collapse" class="btn nodjax"><?php echo JText::_('COM_FINDER_ADVANCED_SEARCH_TOGGLE'); ?></a>
		<?php endif; ?>-->
	</fieldset>
